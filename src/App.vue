<template>
  <div :class="['widget', { 'widget--active': isActive }]">
    <button
      class="widget__btn"
      type="button"
    ...
    <div v-if="!loading" class="widget__list">
      ...
      <div v-else>No chosen cities, please select some in settings</div>
    </div>
...
export default defineComponent({
  setup() {
    ...
    const notification = computed(() => store.getters['notification/notification']);

    onMounted(() => {
      if (localStorage.weatherCities) {
        const cities: ICityWeather[] = JSON.parse(localStorage.weatherCities);
    ...
    const citiesWeather = computed<ICityWeather[]>(() => {
